"""
Saved Scans persistence (Phase 5's `scans` table).

Visibility rule: load_scan_for_execution() ALWAYS returns the full
condition set (needed to actually run the scan). get_scan_for_display()
strips the condition set when is_locked=1 and the caller isn't in
Owner Mode - this is the one place the "hide formula from testers"
rule is enforced.
"""
...
from conditions import ConditionSet, condition_set_label, Condition, ConditionEntry
...
def _condition_set_to_json(condition_set: ConditionSet) -> str:
    return json.dumps([entry.to_dict() for entry in condition_set])


def _condition_set_from_json(raw: str) -> ConditionSet:
    data = json.loads(raw)
...
def save_scan(
    ...
    if existing is None:
        cur = conn.execute(
            """
            INSERT INTO scans
                (name, description, timeframe, condition_set_json, is_locked, is_tracked,
                 scan_version, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
            """,
        ...
        conn.execute(
            """
            UPDATE scans
            SET description = ?, timeframe = ?, condition_set_json = ?,
                is_locked = ?, is_tracked = ?, scan_version = ?, updated_at = ?
            WHERE scan_id = ?
            """,
...
def _row_to_record(row, reveal_condition: bool) -> ScanRecord:
...
def list_scans(conn: sqlite3.Connection, owner_mode: bool) -> List[ScanRecord]:
    cur = conn.execute(
        """
...
def get_scan_for_display(conn: sqlite3.Connection, name: str, owner_mode: bool) -> ScanRecord:
    cur = conn.execute(
        """
        SELECT scan_id, name, description, timeframe, condition_set_json,
               is_locked, is_tracked, scan_version, created_at, updated_at
...
def load_scan_for_execution(conn: sqlite3.Connection, name: str):
    cur = conn.execute(
        "SELECT timeframe, condition_set_json FROM scans WHERE name = ?", (name,)
    )
    row = cur.fetchone()
    if row is None:
        raise ScanRepositoryError(f"No saved scan named '{name}'")
    timeframe, condition_json = row
    return timeframe, _condition_set_from_json(condition_json)


def rename_scan(conn: sqlite3.Connection, old_name: str, new_name: str) -> None:
...
def delete_scan(conn: sqlite3.Connection, name: str) -> None:
...
def set_tracked(conn: sqlite3.Connection, name: str, is_tracked: bool) -> None:
    cur = conn.execute(
...
def list_tracked_scan_names(conn: sqlite3.Connection) -> List[str]:
    cur = conn.execute("SELECT name FROM scans WHERE is_tracked = 1 ORDER BY name")
